...
export const generateFullSpec = <T extends Record<string, string | undefined>>(
  baseSpec: T,
  detailSpec: Partial<T>,
  normalize = true
): T => {
  const fullSpec: T = { ...baseSpec };

  for (const key in detailSpec) {
    if (fullSpec[key] && fullSpec[key].trim().endsWith('!preserve')) continue;
    fullSpec[key] = detailSpec[key];
  }

  if (!normalize) return fullSpec;

  for (const key in fullSpec) {
    if (!fullSpec[key]) continue;
    fullSpec[key] = fullSpec[key].replace('!preserve', '') as T[typeof key];
  }
...
export const transformCSSKey = (key: string) => {
  let result = '';
...
export const normailzeInteractionState = (state: string) => {
  if (state === 'press') state = 'active';
  if (state === 'focus') state = 'focus:not(:active)';
  if (state !== 'disabled') state += ':not(:disabled)';
  return state;
};
